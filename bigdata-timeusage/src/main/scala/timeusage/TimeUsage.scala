...
object TimeUsage {
  ...
  def timeUsageByLifePeriod(): Unit = {
    ...
    val finalds = timeUsageGroupedTyped(ds)

//    val dfarray = finalDf.collect()
//    val sqlarray = sqldf.collect()
//    val dsarray = finalds.collect()
  ...
  def fsPath(resource: String): String = {
    val extracted = getClass.getResource(resource).toURI
    Paths.get(extracted).toString
  }
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val temp = columnNames.indices.map(index => {
      if (index == 0)
        StructField(columnNames(index), StringType, false)
      else
        StructField(columnNames(index), DoubleType, false)
    })
    StructType(temp)
  }
  ...
  def row(line: List[String]): Row = {
    ...
    val temp = for (i <- line.indices) yield {
      if (i == 0)
        line(i).replaceAll("\"", "")
      else
        line(i).toDouble
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] = {
    val a = timeUsageSummaryDf.as[TimeUsageRow]
    a
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    val k = summed.groupByKey(x => (x.working, x.sex, x.age)).agg(typed.avg(_.primaryNeeds), typed.avg(_.work), typed.avg(_.other))
    val x = k.map(x => TimeUsageRow(x._1._1, x._1._2, x._1._3, myround(x._2), myround(x._3), myround(x._4)))
    x.orderBy($"working", $"sex", $"age")
  }

  def myround(d: Double): Double = Math.round(d * 10.0) / 10.0
}
